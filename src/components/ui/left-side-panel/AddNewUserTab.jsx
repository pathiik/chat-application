...
function AddNewUserTab({ profilePicure, name, username }) {
  ...
  return (
    <>
      <div className="px-2 py-3 mt-2 mr-1 border-b">
        <form className="flex flex-col gap-2 items-center justify-center" onSubmit={handleNewChat}>
          <div className="flex items-center gap-3 w-full">
            <img
              className="h-12 w-12 border-2 border-solid border-gray-800 rounded-full"
              src={profilePicure || "https://static.vecteezy.com/system/resources/previews/002/275/847/non_2x/male-avatar-profile-icon-of-smiling-caucasian-man-vector.jpg"}
              alt=""
            />
            <div className="w-full flex justify-between">
              <div className="flex flex-col overflow-x-hidden" title="Pathik">
                <p className="font-semibold w-40">{name}</p>
                <p className="text-gray-600 w-40">@{username}</p>
              </div>
            </div>
          </div>
